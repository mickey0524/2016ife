...
Game.prototype.init = function() {
    this.background.start(this.guardNum, [], 0);
...
Game.prototype.animate = function() {
    ...
    //     this.restart();
    // }
    if(this.collision(this.people, this.guard)) {
        this.restart();
    }
    else {
...
/**
 * 判断人物是否走过地图的一半高度，是的话，重新生成
 */
Game.prototype.infinityGenerate = function() {
    var target = 2 * parseInt(map.columns / 3) + 1;
    var self = this;
    if(this.people.y > target * map.ceilHeight) {
        var personX = this.people.x;
        var personY = canvasHeight - this.people.y;
        var personAngle = this.people.rotateAngle;
        var backgroundArray = [];
        for(var i = 0; i < map.rows; i++) {
            var arr = [];
            for(var j = 0; j <= map.columns - target + 1; j++) {
                arr.push(map.arr[i][j + target - 2]);
            }
            backgroundArray.push(arr);
        }
        this.context.clearRect(0, 0, canvasWidth, canvasHeight);
        this.background.context.clearRect(0, 0, canvasWidth, canvasHeight);
        this.guard = [];
        this.background.start(this.guardNum, backgroundArray, map.columns - target + 2);
        this.people.init(parseInt(personX / map.ceilWidth), parseInt(personY / map.ceilHeight));
        for(var i = 0; i < map.rows; i++) {
            for(var j = 0; j < map.columns; j++) {
                if(map.arr[i][j] == 3) {
                    var guard = new Guard(this.context, this.imageFactory, i * map.ceilWidth, j * map.ceilHeight, 30, 30);
    ...
    var infinityTimer = setTimeout(function() {
        self.infinityGenerate();
    }, 1000);         
}