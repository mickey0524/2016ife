...
Background.prototype.start = function(guardNum) {
    ...
    this.makeGuard(this.guardNum * 3);
    this.makeBomb(3);
    this.draw();
}
 ...
 * @Param {int} num 守卫个数
 */
Background.prototype.makeGuard = function(num) {
    for(var i = 0; i < num; i++) {
        var x = parseInt(Math.random() * map.rows);
        var y = parseInt(Math.random() * map.columns);
 ...
 * @Param {int} num 炸弹个数
 */
Background.prototype.makeBomb = function(num) {
    for(var i = 0; i < num; i++) {
        var x = parseInt(Math.random() * map.rows);
        var y = parseInt(Math.random() * map.columns);
        if(map.arr[x][y] != 0 ||  x == map.rows - 1 || y == map.columns - 1 || x == 0 || y == 0) {
            i -= 1;
            continue;
        }
        if(map.arr[x + 1][y] == 1 && map.arr[x - 1][y] == 1 &&
           map.arr[x][y - 1] == 1 && map.arr[x][y + 1] == 1) {
            i -= 1;
            continue;
        }
        else {
