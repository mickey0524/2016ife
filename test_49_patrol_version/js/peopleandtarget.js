...
PeopleAndTarget.prototype.init = function(x, y) {
    this.x = x;
    this.y = y;
    this.context.drawImage(this.imageFactory.people, x, y, this.width, this.height);
...
PeopleAndTarget.prototype.move = function() {
    ...
    if(keyStatus.left || keyStatus.right || keyStatus.top || keyStatus.bottom) {
        ...
        if(keyStatus.top) {
            ...
            else if(this.y <= 0 || this.isObstacle(this.x, this.y)) {
                this.y += this.speed;
                this.score += this.speed;
            }
            ...
            if(this.y >= canvasHeight - this.height || this.isObstacle(this.x, this.y)) {
                ...
                if(this.y - Math.abs(Number(top)) > 2 * (document.documentElement.offsetHeight / 3)) {
                    if(Math.abs(Number(top)) <= (document.documentElement.offsetHeight - 30) * 4) {
                        document.getElementById('background').style.top = (Number(top) - this.speed) + 'px';
                        document.getElementById('people').style.top = (Number(top) - this.speed) + 'px';                        
                    }
                }
            }
            this.rotateAngle = 90;
